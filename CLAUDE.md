...
Single-binary Go CLI with 844 embedded markdown cheatsheets and 737 deep-dive theory pages across 65 categories. Built-in calculator (unit-aware), subnet calculator, fuzzy search, interactive TUI, REST API daemon, shell completions, bookmarks, cross-references, export, learning paths, math verification. Covers 13 certification domains (CCNP DC/Enterprise, CCIE EI/SP/Security/Automation, JNCIE-SP/SEC, Linux+, CISSP, C|RAGE, AWS SAA-C03 in `aws/`, Security+ SY0-701 in `security-plus/`) plus the `ramp-up/` curriculum (55 ELI5-voiced sheets — kernel, all major network protocols, security/auth, observability, IaC, CI/CD, languages, databases, web servers, cloud, daily-use tools, fundamental networking concepts, network automation).
...
- `sheets/<category>/<topic>.md` — 844 embedded cheatsheets across 65 categories
